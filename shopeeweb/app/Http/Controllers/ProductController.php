...
class ProductController extends Controller
{
    ...
    public function save_product(Request $request) {
        ...
        $data['brand_product_id'] = $request->product_brand;
        $data['product_status'] = $request->product_status;
        $get_image = $request->file('product_image');
        //Xử lí image
        if($get_image){
            $get_name_image = $get_image->getClientOriginalName();
            // hàm xử lý bỏ đuôi.jpg .png
            $name_image = current(explode('.',$get_name_image));
            $today = date('Y-m-d');
            $new_image = $name_image.rand(0,99). '.' .$today. '.' .$get_image->getClientOriginalExtension();
    ...
    public function change_status($product_id) {
        $this->AuthLogin();
        $get_product_id = DB::table('tbl_product')->where('product_id',$product_id)->first();
        if($get_product_id && $get_product_id->product_status == 1) {
    ...
    public function delete_product($product_id) {
        $this->AuthLogin();
        $get_id = DB::table('tbl_product')->where('product_id',$product_id)->first();
        if($get_id) {
